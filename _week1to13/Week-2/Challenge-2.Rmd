...
**Solution:** _"csv" stands for comma seperated values and is popular for it's readability and ease of creating and editing by human standards, not to mention its compatibility with many different editing softwares further feeds into the feedback loop of its popularity and wide use_ 
<br>
<br>
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="I scrolled down"}
knitr::include_graphics("Screenshot (3).png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** _It leaves out designated rows from the dataset, you can use it to leave out rows with missing information_ 
<br>
<br>
...
ggplot(data=PD, mapping=aes(x=Age,y=Rating)) 
...
ggplot(data=PD, mapping=aes(x=Age,y=Rating)) + geom_point()
...
ggplot(data=PD, mapping=aes(x=Age,y=Rating)) + geom_point(shape=4)
...
ggplot(data=PD,mapping=aes(x=Age,y=Rating)) + 
  geom_point(shape=4) + 
  labs(x="Age (years)",y="Rating (Points)", 
                                                                          title="Dj ratings across different ages",
                                                                          caption="Too scattered to form a concrete relation between age and rating")
